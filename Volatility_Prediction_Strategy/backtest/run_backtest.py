...
def main():
	...
	if res_cfg.get("write_overview_html", True):
		try:
			...
			rows = 4
			row_height = int(viz_cfg.get("row_height", 500))
			fig = make_subplots(rows=rows, cols=1, shared_xaxes=True, vertical_spacing=0.03,
				subplot_titles=("Underlying Kline with Signals", "Equity Curve", "ATM IV vs Forward RV (+/- epsilon)", "Signal"), specs=[[{"secondary_y": False}]]*rows)
			...
			if underlying is not None and set(["open","high","low","close"]).issubset(underlying.columns):
				...
				if len(trades) > 0:
					...
					fig.add_trace(go.Scatter(x=entries["open_date"], y=entries["open_date"].map(ud), mode="markers", name="Entry", marker=dict(color="#2ca02c", size=10, symbol="triangle-up")), row=1, col=1)
					fig.add_trace(go.Scatter(x=exits["close_date"], y=exits["close_date"].map(ud), mode="markers", name="Exit", marker=dict(color="#d62728", size=10, symbol="triangle-down")), row=1, col=1)
			...
			fig.update_layout(title="Volatility Strategy Backtest Overview", xaxis_rangeslider_visible=False, template="plotly_white", height=row_height * rows)
