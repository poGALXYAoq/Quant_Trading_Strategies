...
# 新增参数：指定希望进行交易的合约月份列表
TRADE_CONTRACT_MONTHS = [1, 5, 10] # 例如: [1, 5, 9] 表示只交易近月合约是1月、5月、9月的价差组合
...
RESULTS_DIR = 'backtest_results_159/RB' # 修改结果目录以防覆盖
...
def prepare_data(file_path):
    ...
    # --- 新增逻辑: 从合约代码中提取合约月份 ---
    # 假设合约代码格式为 "品种+YYMM+..." 或 "品种+YMM+...", 例如 RB2405 或 TA003
    contract_code_series = df[col2].str.extract(r'^[A-Z]+(\d+)')
    # 删除无法提取合约代码的行
    df.dropna(subset=[col2], inplace=True)
    valid_codes_mask = contract_code_series[0].notna()
    df = df[valid_codes_mask]
    contract_codes = contract_code_series[valid_codes_mask]
    # 从代码中取最后两位作为月份
    df['contract_month'] = contract_codes[0].str[-2:].astype(int)
    # --- 结束新增逻辑 ---
...
def generate_trade_schedule(df, N):
    ...
    for expiry in sorted(potential_near_expiries):
        ...
        # --- 新增逻辑: 检查近月合约的月份是否在允许的列表中 ---
        near_contract_month = near_month_options_on_open_date['contract_month'].iloc[0]
        if TRADE_CONTRACT_MONTHS and near_contract_month not in TRADE_CONTRACT_MONTHS:
            continue
        # --- 结束新增逻辑 ---
...
def find_opening_trade_event(open_date_options, near_month_expiry, k, underlying_open_price):
    ...
    # 找到近月要卖出的合约
    sell_call_near_row = near_calls.iloc[(near_calls['行权价'] - target_strike_call).abs().argsort()[:1]]
    sell_put_near_row = near_puts.iloc[(near_puts['行权价'] - target_strike_put).abs().argsort()[:1]]
    if sell_call_near_row.empty or sell_put_near_row.empty:
        return []
    
    sell_call_near = sell_call_near_row.iloc[0]
    sell_put_near = sell_put_near_row.iloc[0]
    
    # 3. 在远月合约中，寻找与卖出合约相同行权价的合约进行买入
    call_strike_to_match = sell_call_near['行权价']
    put_strike_to_match = sell_put_near['行权价']

    far_calls = far_month_options[far_month_options['期权类型'] == 'C']
    far_puts = far_month_options[far_month_options['期权类型'] == 'P']

    buy_call_far_row = far_calls[far_calls['行权价'] == call_strike_to_match]
    buy_put_far_row = far_puts[far_puts['行权价'] == put_strike_to_match]

    if buy_call_far_row.empty or buy_put_far_row.empty:
        return []

    buy_call_far = buy_call_far_row.iloc[0]
    buy_put_far = buy_put_far_row.iloc[0]
    
    return [
        (sell_call_near, 'short'),
        (sell_put_near, 'short'),
        (buy_call_far, 'long'),
        (buy_put_far, 'long')
    ]
    ...
    for today in all_trading_days:
        if portfolio and today == current_close_date:
            ...
            for code, pos_to_close in portfolio.items():
                # 明确计算平仓价(exit_price), 其计算逻辑与开仓价完全独立
                exit_price = -1
                if not close_date_options.empty and code in close_date_options.set_index(col2).index:
                    # 优先使用平仓日当天的结算价
                    exit_price = close_date_options.set_index(col2).loc[code]['settlement_price']
                else:
                    # 若平仓日无数据(通常因为到期), 则计算内在价值作为平仓价
                    strike_price = pos_to_close['info']['行权价']
                    option_type = pos_to_close['info']['期权类型']
                    exit_price = max(0, underlying_price_at_close - strike_price) if option_type == 'C' else max(0, strike_price - underlying_price_at_close)

                pnl = (exit_price - pos_to_close['entry_price'] if pos_to_close['direction'] == 'long' else pos_to_close['entry_price'] - exit_price) * pos_to_close['lots']
                
                cash_before_this_trade = cash
                if pos_to_close['direction'] == 'long':
                    cash += exit_price * pos_to_close['lots']
                else:
                ...
                detailed_trade_log.append({
                    ...
                    'entry_price': pos_to_close['entry_price'], 'exit_price': exit_price,
